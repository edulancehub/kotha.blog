    ...
    const timer = setTimeout(() => {
      startCheck(async () => {
        ...
        if (result.available) {
          ...
          setCheckStatus(result.reason?.includes("reserved") || result.reason?.includes("Invalid") ? "invalid" : "taken");
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-gradient-to-br from-teal-600 via-emerald-600 to-cyan-600 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative max-w-md text-white animate-fade-in">
          ...
          <h1 className="font-serif text-4xl font-bold tracking-tight leading-tight">
            Your words deserve<br />their own address
          </h1>
          <p className="mt-4 text-base text-teal-100 leading-relaxed">
            Create a beautiful blog, claim your personal subdomain, and join a community of writers.
          </p>
          <div className="mt-10 space-y-4">
            {[
              ...
              <div key={text} className="flex items-center gap-3 text-teal-50 text-sm">
        ...
        <div className="w-full max-w-[26rem] animate-fade-in">
          ...
          <form action={formAction} className="mt-7 space-y-4">
            <div>
              ...
              <input id="displayName" name="displayName" type="text" required className="input-field" placeholder="Your Name" />
            </div>

            <div>
              <label htmlFor="username" className="input-label">Username (subdomain)</label>
              <div className="flex items-stretch">
                ...
                />
                <span className="flex items-center rounded-r-xl border border-l-0 border-border bg-surface px-3 text-sm text-muted whitespace-nowrap">
                  .kotha.blog
                </span>
              </div>
              {/* Live status */}
            ...
            <button
              type="submit"
              disabled={pending || checkStatus === "taken" || checkStatus === "invalid"}
              className="btn-primary w-full py-3 text-[0.9375rem] mt-2"
            >
              {pending ? (
